...
// of this type are "correct" schemas as defined by the JDDF spec. To verify the
...
// Form represents the eight kinds of JDDF schemas defined by the spec.
//
...
func (s *Schema) verify(root *Schema) error {
	...
	if s.Type != "" {
		...
		switch s.Type {
		case "boolean", "float32", "float64", "int8", "uint8", "int16", "uint16",
			"int32", "uint32", "int64", "uint64", "string", "timestamp":
		default:
	...
	if s.Discriminator.Tag != "" || s.Discriminator.Mapping != nil {
		if !isEmpty {
			return ErrInvalidForm
		}

		if s.Discriminator.Mapping == nil {
			return ErrMissingDiscriminatorMapping
		}
